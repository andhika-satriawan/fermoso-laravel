...
<div class="page-header">
    ...
    <div class="page-btn">
      <a href="{{ route('admin.faq.list.create') }}" class="btn btn-added">
        <img
                    ...
                    <tr>
                        ...
                        <td class="productimgname">
                            <a href="javascript:void(0);" class="product-img">
                                <img
                                    src="{{ \Storage::disk('local')->url($faq->image) }}"
                                    alt="product"
                                />
                            </a>
                        </td>
                        <td>{{ $faq->title }}</td>
                        <td>
                            {{ \Illuminate\Support\Arr::join(Arr::pluck($faq->categories, 'name'), ', ', ' and ') }}
                        </td>
                        ...
                        <td>{{ \Carbon\Carbon::createFromFormat('Y-m-d H:i:s', $faq->created_at)->format('M d, Y, H:i:s') }}</td>
                        <td>
                            <a class="me-3" href="{{ route('admin.faq.list.edit', $faq->id)}}">
                                <img src="{{ asset('admin/img/icons/edit.svg')}}" alt="img" />
                            </a>
                            <a class="me-3 confirm-text" onclick="deleteConfirmation({{ $faq->id }})">
    ...
    const deleteConfirmation = (id) => {
        Swal.fire({
            ...
            if (result.isConfirmed) {
                ...
                $.ajax({
                    type: 'DELETE',
                    url: '{{ route("admin.faq.list.index") }}/' + id,
                    data: {
