...
<div class="page-header">
    ...
    <div class="page-btn">
      <a href="{{ route('admin.article.blog.create') }}" class="btn btn-added">
        <img
                    ...
                    <tr>
                        ...
                        <td class="productimgname">
                            <a href="javascript:void(0);" class="product-img">
                                <img
                                    src="{{ \Storage::disk('local')->url($article->image) }}"
                                    alt="product"
                                />
                            </a>
                        </td>
                        <td>{{ $article->title }}</td>
                        <td>
                            {{ \Illuminate\Support\Arr::join(Arr::pluck($article->categories, 'name'), ', ', ' and ') }}
                        </td>

                        @if ($article->status == 1)
                            <td><span class="text-success">ACTIVE</span></td>
                        @elseif ($article->status == 2)
                            <td><span class="text-danger">DEACTIVATED</span></td>
                        @else
                            <td><span class="text-warning">DRAFT</span></td>
                        @endif
                        
                        <td>{{ \Carbon\Carbon::createFromFormat('Y-m-d H:i:s', $article->created_at)->format('M d, Y, H:i:s') }}</td>
                        <td>
                            <a class="me-3" href="{{ route('admin.article.blog.edit', $article->id)}}">
                                <img src="{{ asset('admin/img/icons/edit.svg')}}" alt="img" />
                            </a>
                            <a class="me-3 confirm-text" onclick="deleteConfirmation({{ $article->id }})">
    ...
    const deleteConfirmation = (id) => {
        Swal.fire({
            ...
            if (result.isConfirmed) {
                ...
                $.ajax({
                    type: 'DELETE',
                    url: '{{ route("admin.article.blog.index") }}/' + id,
                    data: {
