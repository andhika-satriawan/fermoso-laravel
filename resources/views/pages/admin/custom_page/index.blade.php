...
<div class="page-header">
    ...
    <div class="page-btn">
      <a href="{{ route('admin.custom-page.create') }}" class="btn btn-added">
        <img
                    ...
                    <tr>
                        ...
                        <td class="productimgname">
                            <a href="javascript:void(0);" class="product-img">
                                <img
                                    src="{{ \Storage::disk('local')->url($custom_page->image) }}"
                                    alt="product"
                                />
                            </a>
                        </td>
                        <td>{{ $custom_page->title }}</td>
                        <td>{{ $custom_page->code }}</td>
                        <td>{{ \Carbon\Carbon::createFromFormat('Y-m-d H:i:s', $custom_page->created_at)->format('M d, Y, H:i:s') }}</td>
                        <td>
                            <a class="me-3" href="{{ route('admin.custom-page.edit', $custom_page->id)}}">
                                <img src="{{ asset('admin/img/icons/edit.svg')}}" alt="img" />
                            </a>
                            <a class="me-3 confirm-text" onclick="deleteConfirmation({{ $custom_page->id }})">
    ...
    const deleteConfirmation = (id) => {
        Swal.fire({
            ...
            if (result.isConfirmed) {
                ...
                $.ajax({
                    type: 'DELETE',
                    url: '{{ route("admin.custom-page.index") }}/' + id,
                    data: {
