...
@section('content')
    ...
    <div class="card upload-product">
        <div class="card-body">
            <form action="{{ route('admin.review.update', $item->id) }}" id="formSubmission" method="post" id="" enctype="multipart/form-data">
            @csrf
            @method('PATCH')
                <div class="row">
                    ...
                    <div class="col-lg-12 col-12">
                        <div class="form-group">
                            ...
                            <input type="text" class="form-control @error('customer_name') is-invalid @enderror" id="customer_name" placeholder="Customer Name" name="customer_name" value="{{ $item->customer_name }}" autocomplete="off" required>
                            @error('customer_name')
                            <div class="invalid-feedback">
                                {{ $message }}
                            </div>
                        ...
                        <div class="form-group">
                            ...
                            <select class="select @error('rating') is-invalid @enderror"
                                    ...
                                    @foreach (collect([1, 2, 3, 4, 5]) as $rating)
                                        <option @if ($item->rating == $rating) selected @endif
                                            value="{{ $rating }}">
                                            {{ $rating }} 
                    ...
                    <div class="col-lg-12 col-12">
                        <div class="form-group">
                            ...
                            <textarea id="comment" name="comment" class="form-control @error('comment') is-invalid @enderror" placeholder="Comment" required>{{ $item->comment }}</textarea>
                            @error('comment')
                            <div class="invalid-feedback">
                                {{ $message }}
                            </div>
